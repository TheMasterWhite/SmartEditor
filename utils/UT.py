...
def Test():
    a = input()
    absPath = FileProcess.AbsPath("resources/亚托莉.txt")
    lib = FileProcess.ReadTxt(absPath)
    result = LLMInterface.Check_String(a, lib)
    print(result)
...
def SaveMoney():
    ...
    for i in FileIterator():
        content = OCRInterface.GetRawOCRResult(FilePath = i)
        result += content
    prompt += result
